...
if (walletBalance < minDeposit) {
  ...
  const depositTx = await synapse.payments.deposit(TOKENS.USDFC, minDeposit, {
    onAllowanceCheck: (current, required) => {
      console.log(
        `Allowance check: ${ethers.formatUnits(current, 18)} / ${ethers.formatUnits(required, 18)} USDFC`
      );
    },
...
- **[Storage Operations](/synapse-sdk/storage/)** - Complete guide to uploading, downloading, and managing datasets
- **[Payment Management](/synapse-sdk/payments/)** - Learn about deposits, approvals, and monitoring costs
- **[Usage Monitoring](/synapse-sdk/payments/storage-monitoring/)** - Track your storage ingress and egress
- **[Core Concepts](/core-concepts/)** - Understand the architecture, PDP, and FilecoinPay protocols

### Developer Resources

- **[Best Practices](/synapse-sdk/best-practices/)** - Production deployment guidelines
- **[Example Application](https://github.com/filecoin-project/fs-upload-dapp)** - Full-stack upload application
